#include <stdio.h>
#include <stdlib.h>
#include <locale.h> //Biblioteca para pegar acentuações
#include <conio.h>// Biblioteca para utilizar funções específicas, no caso deste código a função "getch"
...
#define numeroUSER 100 // Constante para que todas as variaveis delimitadoras sejam fáceis de manusear
...
int senha[numeroUSER] = {1234, 1234, 1234}; // -2147483648 e 2147483647

//inicialização das funções
int menu(int id_us); //Retorna valor para o código
int logar(void); // Retorna valor para o código
int cadastro(void); // Retorna valor para o código
void saque(int id_us); // NÃO Retorna valor para o código
void transferencia(int id_us); // NÃO Retorna valor para o código
void deposito(int id_us); // NÃO Retorna valor para o código

//Variáveis para a função de cadastro
int cadastrados = 2;

//Variável para correção do erro na opção de sair, informando
//erro de conta ou senha incorreta junto, causado pela linha 155
// essa verificação faz com que não ocorra o erro da linha 155 ao sair de algum usuário
int erroSair = 0;
...
int main()
{
  ...
  while (tentativas < 3)
  {
    ...
    if (indice != -1)
    {
      ...
      case 0:
        indice = -1; //indice -1 é verificado na linha 68 para procedencia de logon
        entradaOk = 0;
        break;
      default:
        printf("\nEscolha incorreta!!!");
...
{
  float valor_tr = 0;
  int conta_tr, agencia_tr, id_ustr;// Informações do usuário beneficiado
  int erro = 1;
  ...
  case 0:
    ...
    printf("\nSeu novo saldo é de R$ %.2f", saldo[id_us]);
    entradaOk = 1;
    break;
  case 1:
...
int cadastro()
{
  ...
  scanf("%[a-z A-Z]s", &nome[cadastrados]);
